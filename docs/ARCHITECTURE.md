...
- shading grids / binary shaded-cell overlays / annual heatmap cells
...
- `src/app/analysis/*`: derived computation boundary for solved roofs, shading mode, shading overlay state, annual simulation, sun-projection derived state, and diagnostics.
...
- `src/rendering/*`: reusable Three/MapLibre rendering primitives, shared world-geometry adapters, and Three.js renderer lifecycle helpers.
...
- `src/app/features/map-editor/MapObjects/*`: rendered roof/obstacle meshes and roof-projected binary shaded-cell custom-layer synchronization.
...
- Shading requests are memoized by deterministic geometry/time fingerprints and computed outside render via async state transitions (`PENDING` / `STALE` / `READY`).
- Annual simulation supports batched yielding and optional half-year mirroring to reduce compute cost.
- Binary shaded-cell overlays render as a simple monochrome custom layer projected onto the roof surface; geometry sync failures surface through existing map/runtime error handling.
...
- Operational failures use typed app errors (`code`, `severity`, `recoverable`, `context`) and `Result<T, E>` at storage/share/mesh/map-layer boundaries.
...
- Binary shaded-cell overlay sync failures are handled by existing map/runtime error reporting.
