...
import { buildHeatmapGeometry } from './heatmapGeometry'
...
function feature(intensity: number): RoofHeatmapFeature {
  return {
    type: 'Feature',
    properties: {
      roofId: 'roof-1',
...
function overlappingFeature(roofId: string): RoofHeatmapFeature {
  return {
    type: 'Feature',
    properties: {
      roofId,
