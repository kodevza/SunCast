...
vi.mock('../project-store/useProjectStore', () => ({
  useProjectStore: () => mockUseProjectStore(),
}))

vi.mock('../analysis/useSolvedRoofEntries', () => ({
  useSolvedRoofEntries: (...args: unknown[]) => mockUseSolvedRoofEntries(...args),
}))

vi.mock('../hooks/useConstraintEditor', () => ({
  useConstraintEditor: () => ({
...
vi.mock('../analysis/useSunProjectionState', () => ({
...
vi.mock('../analysis/useRoofShading', () => ({
  useRoofShading: (...args: unknown[]) => mockUseRoofShading(...args),
}))
vi.mock('../analysis/useAnnualRoofSimulation', () => ({
...
describe('useSunCastController', () => {
  ...
  it('routes selection/edit actions through session boundary commands', () => {
    ...
    const hook = renderController()
    act(() => {
      hook.get().canvasModel.mapView.selection.onSelectFootprint('a', false)
      hook.get().canvasModel.mapView.selection.onSelectFootprint('b', true)
    })
  ...
  it('disables production computation while geometry drag is active', () => {
    ...
    act(() => {
      hook.get().canvasModel.mapView.view.onGeometryDragStateChange(true)
    })
    ...
    act(() => {
      hook.get().canvasModel.mapView.view.onGeometryDragStateChange(false)
    })
